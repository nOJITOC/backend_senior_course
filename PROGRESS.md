# Progress Tracker

### Phase 1: Foundations & Kotlin Backend Basics

- [x] [1. Modern Backend Architecture](./lessons/01-foundations/01-modern-backend-architecture.md)
- [x] 2. Setting up the Kotlin Backend Environment ([completed](./lessons/01-foundations/02-setting-up-kotlin-backend-environment.md))
- [ ] 3. Introduction to Ktor
- [ ] 4. Asynchronous Programming with Kotlin Coroutines

### Phase 2: Database & Core Logic

- [ ] 5. Database Fundamentals
- [ ] 6. Database Schema Design
- [ ] 7. Data Persistence in Kotlin
- [ ] 8. Authentication & Authorization

### Phase 3: Game Mechanics & External APIs

- [ ] 9. Implementing Core Game Logic
- [ ] 10. Integrating with Google Maps Platform
- [ ] 11. Integrating with Google's Generative Language API
- [ ] 12. Real-time Communication with WebSockets

### Phase 4: Introduction to Go for High-Performance Services

- [ ] 13. Go Language Fundamentals
- [ ] 14. Building a High-Performance Microservice in Go
- [ ] 15. Inter-service Communication

### Phase 5: Senior-Level Topics: Production Readiness

- [ ] 16. Containerization with Docker
- [ ] 17. Caching Strategies with Redis
- [ ] 18. Comprehensive Testing
- [ ] 19. Continuous Integration & Continuous Deployment (CI/CD)
- [ ] 20. Observability

### Phase 6: Monetization & Final Deployment

- [ ] 21. Implementing Monetization
- [ ] 22. Cloud Deployment & Finalization